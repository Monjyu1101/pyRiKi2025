...
class _perplexityAPI:
    ...
    def authenticate(self, api,
        ...
        if (v_nick_name != ''):
            ...
            if (v_model not in self.models):
                self.models[v_model] = {"id": v_model, "token": str(v_token), "modality": "text?", "date": ymd, }
            else:
        ...
        if (x_nick_name != ''):
            ...
            if (x_model not in self.models):
                self.models[x_model] = {"id": x_model, "token": str(x_token), "modality": "text?", "date": ymd, }
            else:
    ...
    def run_gpt(self, chat_class='chat', model_select='auto',
        ...
        # APIリクエスト用メッセージの生成
        msg = bot_common.history2msg_perplexity(history=res_history)
        ...
        # 最終応答結果の確認
        if (res_text != ''):
            logger.info(f"//Perplexity// {res_name.lower()}, complete.")
        else:
    ...
    def chatBot(self, chat_class='auto', model_select='auto',
        ...
        # モデル実行
        logger.debug(f"//Perplexity// chat_class={chat_class}, model_select={model_select}")
...
if __name__ == '__main__':
    ...
    perplexityKEY.init(runMode='debug')
    logger.info(f"api_type={perplexityKEY.api_type}")

    # ログキューの設定
    stream_queue = queue.Queue()
    res = perplexityAPI.init(stream_queue=stream_queue)
    ...
    res = perplexityAPI.authenticate('perplexity',
                        ...
                        perplexityKEY.perplexity_key_id,
    ...
    if (res == True):
        ...
        if True:
            logger.info("モデル一覧")
            print(f"----------------------------")
            for model in perplexityAPI.models:
                print(model)
            print(f"----------------------------")
        ...
        if False:
          if function_modules:
            sysText = None
            reqText = ''
            inpText = 'pplx-b,toolsで兵庫県三木市の天気を調べて'
            filePath = []
            if reqText:
                logger.info(f"ReqText : {reqText.rstrip()}")
            if inpText:
        ...
        if False:
            ...
            print(f"----------------------------")
            for history in perplexityAPI.history:
                print(history)
            print(f"----------------------------")
