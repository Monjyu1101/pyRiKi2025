...
class _chatgptAPI:
    ...
    def run_gpt(self, chat_class='chat', model_select='auto',
                    ...
                    try:
                        ...
                        if (response.choices[0].finish_reason == 'tool_calls'):
                            for tool_call in response.choices[0].message.tool_calls:
                                ...
                                f_kwargs = tool_call.function.arguments
                    ...
                    for tc in tool_calls:
                        ...
                        for module_dic in function_modules.values():
                            if (f_name == module_dic['func_name']):
                                hit = True
                                logger.info(f"//ChatGPT//   function_call '{module_dic['script']}' ({f_name})")
                                logger.info(f"//ChatGPT//   → {f_kwargs}")
                                ...
                                # 実行結果を表示
                                logger.info(f"//ChatGPT//   → {res_json}")
                        ...
                        # 関数が見つからない場合
                        if (hit == False):
                            logger.error(f"//ChatGPT//   function not found Error ! ({f_name})")
                            break
        ...
        # 最終応答結果の確認
        if (res_text != ''):
            logger.info(f"//ChatGPT// {res_name.lower()}, complete.")
        else:
    ...
    def chatBot(self, chat_class='auto', model_select='auto',
        ...
        # モデル実行
        logger.debug(f"//ChatGPT// chat_class={chat_class}, model_select={model_select}")
...
if __name__ == '__main__':
    ...
    chatgptKEY.init(runMode='debug')
    logger.info(f"api_type={chatgptKEY.api_type}")

    # ログキューの設定
    stream_queue = queue.Queue()
    res = chatgptAPI.init(stream_queue=stream_queue)
    ...
    res = chatgptAPI.authenticate('chatgpt',
                        ...
                        chatgptKEY.openai_organization, chatgptKEY.openai_key_id,
                        chatgptKEY.azure_endpoint, chatgptKEY.azure_version, chatgptKEY.azure_key_id,
    ...
    if (res == True):
        ...
        if True:
            logger.info("モデル一覧")
            print(f"----------------------------")
            for model in chatgptAPI.models:
                print(model)
            print(f"----------------------------")
        ...
        if True:
            ...
            res_text, res_path, res_files, res_name, res_api, chatgptAPI.history = \
                chatgptAPI.chatBot(chat_class='auto', model_select='auto', 
                                    session_id=session_id, history=chatgptAPI.history, function_modules=function_modules,
        ...
        if False:
            ...
            print(f"----------------------------")
            for history in chatgptAPI.history:
                print(history)
            print(f"----------------------------")
            chatgptAPI.history = []
