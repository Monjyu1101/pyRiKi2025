...
class _openrtAPI:
    ...
    def run_gpt(self, chat_class='chat', model_select='auto',
        ...
        if True:
            ...
            while (function_name != 'exit') and (n < int(max_step)):
                ...
                # モデル実行
                n += 1
                logger.info(f"//OpenRT// {res_name.lower()}, {res_api}, pass={n}, ")
                    ...
                    for tc in tool_calls:
                        ...
                        for module_dic in function_modules.values():
                            if (f_name == module_dic['func_name']):
                                hit = True
                                logger.info(f"//OpenRT//   function_call '{module_dic['script']}' ({f_name})")
                                logger.info(f"//OpenRT//   → {f_kwargs}")
                        ...
                        # 関数が見つからない場合
                        if (hit == False):
                            logger.error(f"//OpenRT//   function not found Error ! ({f_name})")
                            break
        ...
        # 最終応答結果の確認
        if (res_text != ''):
            logger.info(f"//OpenRT// {res_name.lower()}, complete.")
        else:
    ...
    def chatBot(self, chat_class='auto', model_select='auto',
        ...
        # モデル実行
        logger.debug(f"//OpenRT// chat_class={chat_class}, model_select={model_select}")
...
if __name__ == '__main__':
    ...
    openrtKEY.init(runMode='debug')
    logger.info(f"api_type={openrtKEY.api_type}")

    # ログキューの設定
    stream_queue = queue.Queue()
    res = openrtAPI.init(stream_queue=stream_queue)

    # 認証処理
    res = openrtAPI.authenticate('openrouter',
                        openrtKEY.api_type,
    ...
    if (res == True):
        ...
        if True:
            ...
            res_text, res_path, res_files, res_name, res_api, openrtAPI.history = \
                openrtAPI.chatBot(chat_class='auto', model_select='auto', 
                                    session_id=session_id, history=openrtAPI.history, function_modules=function_modules,
        ...
        if False:
            ...
            print(f"----------------------------")
            for history in openrtAPI.history:
                print(history)
            print(f"----------------------------")
            openrtAPI.history = []
